...
const DashBoard: React.FC = () => {
  ...
  const updateCellVoltage = (cellId: number, voltage: number) => {
    ...
    setCSU1Cells((prev) =>
      isCSU1
        ? prev.map((cell) =>
            cell.id === cellId
              ? { ...cell, voltage, status: calculateStatus(voltage, cell.temperature) }
              : cell
          )
        : prev
    );
    ...
    Object.entries(dcCsuResponseData).forEach(([dcIc, cellData]) => {
      Object.entries(cellData).forEach(([cellNo, dataItems]) => {
        ...
        setCells((prev) => {
          ...
          updatedCells[cellIndex] = updatedCell;
          console.log(
            `Renderer: Updated DC CSU cell ${cellId} (DC IC ${dcIc}) with data`,
            updatedCell
          );
          return updatedCells;
        });
  ...
  const updateCellState = (instruction: SetInstruction) => {
    ...
    setCells((prev) => {
      ...
      updatedCells[cellIndex] = updatedCell;
      console.log(
        `Renderer: Updated cell ${cellNo} for command ${instruction.command}`,
        updatedCell
      );
      return updatedCells;
    });
