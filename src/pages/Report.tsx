...
//need an update for individual cell data
// import React, { useMemo, useRef, useEffect } from "react";
// import CustomTitleBar from "../components/MenuBar";
// import { useBatteryContext } from "../BatteryContext";
// import jsPDF from "jspdf";
...
//   // Process cell data with response data and instructions
//   const cells = useMemo(() => {
//     const safeCellData = Array.isArray(cellData)
//       ? cellData
//       : Array.from({ length: 24 }, (_, i) => ({
...
//     const computedCells = safeCellData.map((cell) => {
//       const cellDataResponse = [
//         ...(Array.isArray(csu1ResponseData[cell.id]) ? csu1ResponseData[cell.id] : []),
//         ...(Array.isArray(csu2ResponseData[cell.id]) ? csu2ResponseData[cell.id] : []),
//         ...(Array.isArray(dcCsuResponseData[cell.id]) ? dcCsuResponseData[cell.id] : []),
//       ];
...
//       let csu11Voltage: number | null = cell.csu11Voltage;
//       let csu11Temperature: number | null = cell.csu11Temperature;
//       let csu11Balance: boolean = cell.csu11Balance;
//       let csu11OpenWire: boolean = cell.csu11OpenWire;
//       let csu12Voltage: number | null = cell.csu12Voltage;
//       let csu12Temperature: number | null = cell.csu12Temperature;
//       let csu12Balance: boolean = cell.csu12Balance;
//       let csu12OpenWire: boolean = cell.csu12OpenWire;
//       let dcCsuVoltage: number | null = cell.dcCsuVoltage;
//       let dcCsuTemperature: number | null = cell.dcCsuTemperature;
//       let dcCsuBalance: boolean = cell.dcCsuBalance;
//       let dcCsuOpenWire: boolean = cell.dcCsuOpenWire;
//       let daisyChain: string | null = cell.daisyChain;

//       // Update from responseData
//       if (Array.isArray(cellDataResponse)) {
//         const voltageData = cellDataResponse.find((item) => item.command === 'get_voltage');
//         if (voltageData && voltageData.value) {
//           const parsedVoltage = parseFloat(voltageData.value);
//           if (!isNaN(parsedVoltage)) {
//             if (cell.id < 12) {
//               csu11Voltage = parsedVoltage;
//             } else {
//               csu12Voltage = parsedVoltage;
//             }
//             dcCsuVoltage = parsedVoltage;
//             voltage = parsedVoltage;
//           }
//         }

//         const tempData = cellDataResponse.find((item) => item.command === 'get_temp');
//         if (tempData && tempData.value) {
//           const tempValue = parseFloat(tempData.value.replace(' °C', ''));
//           if (!isNaN(tempValue)) {
//             if (cell.id < 12) {
//               csu11Temperature = tempValue;
//             } else {
//               csu12Temperature = tempValue;
//             }
//             dcCsuTemperature = tempValue;
//             temperature = tempValue;
//           }
//         }
//       }

//       // Update from instructions
...
//       if (balanceInstruction) {
//         const balanceValue = balanceInstruction.value === '1';
//         if (cell.id < 12) {
//           csu11Balance = balanceValue;
//         } else {
//           csu12Balance = balanceValue;
//         }
//         dcCsuBalance = balanceValue;
//         balancing = balanceValue;
//       }

//       const openWireInstruction = cellInstructions.find((instr) => instr.command === 'set_ow');
//       if (openWireInstruction) {
//         const openWireValue = openWireInstruction.value === '1';
//         if (cell.id < 12) {
//           csu11OpenWire = openWireValue;
//         } else {
//           csu12OpenWire = openWireValue;
//         }
//         dcCsuOpenWire = openWireValue;
//         openWire = openWireValue;
//       }

//       // Update daisyChain
//       const daisyData = daisyChainData[cell.id];
//       if (Array.isArray(daisyData)) {
//         const daisyChainItem = daisyData.find((item) => item.command === 'daisy_chain');
//         if (daisyChainItem) {
//           daisyChain = daisyChainItem.value;
//         }
//       }
...
//     console.log('Report: Computed cells:', {
//       totalCells: computedCells.length,
//       filteredCells: computedCells.filter(cell => cell.status !== 'N/A').map(cell => ({
//         id: cell.id,
//         status: cell.status,
//         voltage: cell.voltage,
//         temperature: cell.temperature,
//         setVoltage: cell.setVoltage,
//         csu11Voltage: cell.csu11Voltage,
//         csu12Voltage: cell.csu12Voltage,
//         dcCsuVoltage: cell.dcCsuVoltage,
//         daisyChain: cell.daisyChain,
//       })),
//     });

//     return computedCells;
//   }, [cellData, csu1ResponseData, csu2ResponseData, dcCsuResponseData, instructions, daisyChainData]);
...
//         actual = parseFloat(actualMatch[1]);
//       }
//       const expectedMatch = details.match(/\(Expected:\s*(\d+\.\d+)\s*V\)/) || details.match(/Set Voltage:\s*(\d+\.\d+)/);
//       if (expectedMatch) {
//         console.log(`Report: Parsed expected voltage from "${details}": ${expectedMatch[1]}`);
//         expected = parseFloat(expectedMatch[1]);
//       }
...
//   // Combined statuses
//   const allStatuses = useMemo(() => {
//     const cellStatuses = cells
//       .filter(cell => cell.status !== "N/A")
//       .map(cell => ({
//         type: cell.id < 12 ? "CSU1" : "CSU2",
//         label: `Cell ${cell.id}`,
//         status: cell.status,
//         details: [
//           `Voltage: ${cell.voltage !== null ? cell.voltage.toFixed(2) + " V" : "N/A"}`,
//           `Set Voltage: ${cell.setVoltage !== null ? cell.setVoltage.toFixed(2) + " V" : "N/A"}`,
//           `Temperature: ${cell.temperature !== null ? cell.temperature.toFixed(2) + " °C" : "N/A"}`,
//           `CSU11 Voltage: ${cell.csu11Voltage !== null ? cell.csu11Voltage.toFixed(2) + " V" : "N/A"}`,
//           `CSU12 Voltage: ${cell.csu12Voltage !== null ? cell.csu12Voltage.toFixed(2) + " V" : "N/A"}`,
//           `DC CSU Voltage: ${cell.dcCsuVoltage !== null ? cell.dcCsuVoltage.toFixed(2) + " V" : "N/A"}`,
...
//     console.log("Report: allStatuses:", statuses);
//     return statuses;
//   }, [cells, csu1Statuses, csu2Statuses, daisyStatuses]);
...
//   // Define voltageData using useMemo
//   const voltageData = useMemo(() => {
//     console.log("Report: Generating voltageData from cells and allStatuses");
//     const cellVoltageData = cells
//       .filter(cell => {
//         const actualVoltage = cell.voltage || cell.csu11Voltage || cell.csu12Voltage || cell.dcCsuVoltage;
//         return actualVoltage !== null || cell.setVoltage !== null;
//       })
//       .map(cell => ({
//         label: `Cell ${cell.id} (${cell.id < 12 ? "CSU1" : "CSU2"})`,
//         actualVoltage: cell.voltage || cell.csu11Voltage || cell.csu12Voltage || cell.dcCsuVoltage,
//         setVoltage: cell.setVoltage,
//         status: cell.status,
//       }));

//     const statusVoltageData = allStatuses
//       .filter(status => status.type === "CSU1" || status.type === "CSU2" || status.type === "Daisy Chain")
//       .map(status => {
//         const parsedVoltages = parseValue(status.details, "voltage");
//         return {
...
//     const combinedVoltageData = [
//       ...cellVoltageData,
//       ...statusVoltageData.filter(s => !cellVoltageData.some(c => c.label === s.label)),
//     ].filter(data => data.actualVoltage !== null || data.setVoltage !== null);

//     console.log("Report: voltageData:", combinedVoltageData);
//     return combinedVoltageData;
//   }, [cells, allStatuses]);

//   // Define temperatureData using useMemo
//   const temperatureData = useMemo(() => {
//     console.log("Report: Generating temperatureData from cells and allStatuses");
//     const cellTempData = cells
//       .filter(cell => {
//         const temp = cell.temperature || cell.csu11Temperature || cell.csu12Temperature || cell.dcCsuTemperature;
//         return temp !== null;
//       })
//       .map(cell => ({
//         label: `Cell ${cell.id} (${cell.id < 12 ? "CSU1" : "CSU2"})`,
//         temperature: cell.temperature || cell.csu11Temperature || cell.csu12Temperature || cell.dcCsuTemperature,
//       }));

//     const statusTempData = allStatuses
//       .filter(status => status.type === "CSU1" || status.type === "CSU2" || status.type === "Daisy Chain")
//       .map(status => {
//         const parsedTemp = parseValue(status.details, "temperature");
//         return {
//           label: `${status.label} (${status.type})`,
//           temperature: parsedTemp.actual !== null ? parseFloat(parsedTemp.actual.toFixed(2)) : null,
//         };
//       });

//     const combinedTempData = [
//       ...cellTempData,
//       ...statusTempData.filter(s => !cellTempData.some(c => c.label === s.label) && s.temperature !== null),
//     ].filter(data => data.temperature !== null);

//     console.log("Report: temperatureData:", combinedTempData);
//     return combinedTempData;
//   }, [cells, allStatuses]);

//   // Initialize and update charts
//   useEffect(() => {
//     console.log("Report: useEffect triggered with:", { voltageData, temperatureData, allStatuses });
...
//       const labels = voltageData.map((d) => d.label);
//       console.log("Report: Chart labels:", labels);

//       // Voltage chart
//       if (voltageChartRef.current) {
//         const ctx = voltageChartRef.current.getContext("2d");
//         if (ctx) {
//           console.log("Report: Voltage chart canvas dimensions:", {
//             width: voltageChartRef.current.width,
//             height: voltageChartRef.current.height,
//           });
//           const datasets = [
//             {
//               label: "Actual Voltage (V)",
//               data: voltageData.map((d) => d.actualVoltage ?? 0),
//               borderColor: "rgba(75, 192, 192, 1)",
//               backgroundColor: "rgba(75, 192, 192, 0.2)",
//               fill: true,
//               pointRadius: 5,
...
//             {
//               label: "Expected Voltage (V)",
//               data: voltageData.map((d) => d.setVoltage ?? 0),
//               borderColor: "rgba(255, 99, 132, 1)",
//               backgroundColor: "rgba(255, 99, 132, 0.2)",
//               fill: true,
//               pointRadius: 5,
...
//             },
//           ];
//           console.log("Report: Voltage chart datasets:", datasets);
...
//             },
//           });
//           console.log("Report: Voltage chart created:", voltageChartInstance.current);
//         } else {
//           console.error("Report: Failed to get 2d context for voltage chart");
//         }
//       } else {
//         console.warn("Report: voltageChartRef.current is null");
//       }
...
//         const ctx = tempChartRef.current.getContext("2d");
//         if (ctx) {
//           console.log("Report: Temperature chart canvas dimensions:", {
//             width: tempChartRef.current.width,
//             height: tempChartRef.current.height,
//           });
//           const datasets = [
//             {
//               label: "Temperature (°C)",
//               data: temperatureData.map((d) => d.temperature ?? 0),
//               borderColor: "rgba(75, 192, 192, 1)",
//               backgroundColor: "rgba(75, 192, 192, 0.2)",
//               fill: false,
//             },
...
//             },
//           });
//           console.log("Report: Temperature chart created:", tempChartInstance.current);
//         } else {
//           console.error("Report: Failed to get 2d context for temperature chart");
//         }
//       } else {
//         console.warn("Report: tempChartRef.current is null");
//       }
...
//         const ctx = issueChartRef.current.getContext("2d");
//         if (ctx) {
//           console.log("Report: Issue chart canvas dimensions:", {
//             width: issueChartRef.current.width,
//             height: issueChartRef.current.height,
//           });
//           const issueCounts = [...cells, ...allStatuses].reduce(
//             (acc, item) => {
...
//             },
//           });
//           console.log("Report: Issue chart created:", issueChartInstance.current);
//         } else {
//           console.error("Report: Failed to get 2d context for issue chart");
//         }
//       } else {
//         console.warn("Report: issueChartRef.current is null");
//       }
//     };

//     if (voltageChartRef.current && tempChartRef.current && issueChartRef.current) {
//       console.log("Report: Initializing charts");
//       initializeCharts();
//     } else {
//       console.warn("Report: Waiting for canvas refs");
//       const observer = new MutationObserver(() => {
//         if (voltageChartRef.current && tempChartRef.current && issueChartRef.current) {
//           console.log("Report: Canvas refs available, initializing charts");
//           initializeCharts();
//           observer.disconnect();
//         }
//       });
...
//       }
//     };
//   }, [voltageData, temperatureData, allStatuses]);
...
//   // Check chart visibility
//   const hasVoltageData = useMemo(() => {
//     return voltageData.length > 0;
//   }, [voltageData]);
//   const hasTemperatureData = useMemo(() => {
//     return temperatureData.length > 0;
//   }, [temperatureData]);
//   const hasIssueData = allStatuses.length > 0 || cells.some(cell => cell.status !== "N/A");
//   console.log("Report: Visibility checks:", { hasVoltageData, hasTemperatureData, hasIssueData });
...
//                           {status.status}
//                         </td>
//                         <td className="px-4 py-2 text-sm text-gray-800">{status.details}</td>
//                       </tr>
//                     ))}
//                   </tbody>
//                 </table>
