...
//   const { dcCsuResponseData, instructions } = useBatteryContext();
...
//   // Derive setVoltage from instructions based on cellId
//   const getSetVoltage = (cellId: number) => {
//     console.log('getSetVoltage: Checking instructions', { cellId, instructions });
...
//       return parsedSetVoltage >= 1 && parsedSetVoltage <= 8 ? parsedSetVoltage : null;
//     }
//     return null;
//   };

//   const getSetTemperature = (cellId: number) => {
//     console.log('getSetTemperature: Checking instructions', { cellId, instructions });
//     const cellInstructions = instructions.filter((instr) => parseInt(instr.cellNo) === cellId);
//     console.log('getSetTemperature: Filtered instructions', cellInstructions);
...
//                 {row.map((cellId) => {
//                   const dataItems = dcCsuResponseData && dcCsuResponseData[0]?.[cellId] || [];
...
//                         </div>
//                       </div>
//                       {selectedCell === cellId && dataItems.length > 0 && (
//                         <div
//                           style={popupStyle}
...
const Daicy: React.FC = () => {
  ...
  console.log('dcCsuResponseData:', dcCsuResponseData);
  ...
  // Derive dcIcs dynamically from dcCsuResponseData
  const dcIcs = dcCsuResponseData ? Object.keys(dcCsuResponseData).map(dcIc => parseInt(dcIc)) : [];
  console.log('dcIcs derived:', dcIcs);
  const cellIds = Array.from({ length: 24 }, (_, i) => i); // 12 cells per dcIc
  const rows = [
    cellIds.slice(0, 3),
    cellIds.slice(3, 6),
    cellIds.slice(6, 9),
  ...
  const getSetTemperature = (dcIc: number, cellNo: number) => {
    ...
    console.log('getSetTemperature: Checking', { dcIc, cellNo, overallCellNo, instructions });
    const cellInstructions = instructions.filter((instr) => parseInt(instr.cellNo) === overallCellNo);
    console.log('getSetTemperature: Filtered instructions', cellInstructions);
    const setTempInstruction = cellInstructions.find((instr) => instr.command === 'set_temp');
    if (setTempInstruction && setTempInstruction.temperature) {
              ...
              {rows.map((row, rowIndex) => (
                <div key={rowIndex} className="grid grid-cols-3 gap-2">
                  {row.map((cellNo) => {
                    ...
                    console.log(`Data for DC IC ${dcIc}, Cell ${cellNo}:`, dataItems);
                    if (!dataItems.length) {
                      console.log(`No data for DC IC ${dcIc}, Cell ${cellNo}, skipping render`);
                      return null;
                    }
