...
// //   // take the last cycle for saving (or loop if you want all)
// //   const lastCycle = cycleData[cycleData.length - 1];

// //   const csu11 = Object.entries(lastCycle)
// //     .filter(([_, v]) => v?.csu11)
// //     .map(([cellKey, cellValue], idx) => ({
// //       cell: cellKey,
// //       csu11Voltage: cellValue?.csu11?.receivedVoltage ?? null,
// //       testerVoltage: lastCycle[`cell_${idx + 12}`]?.individual?.receivedVoltage ?? null,
// //     }));

// //   const csu12 = Object.entries(lastCycle)
// //     .filter(([_, v]) => v?.csu12)
// //     .map(([cellKey, cellValue], idx) => ({
// //       cell: cellKey,
// //       csu12Voltage: cellValue?.csu12?.receivedVoltage ?? null,
// //       testerVoltage: lastCycle[`cell_${idx}`]?.individual?.receivedVoltage ?? null,
// //     }));

// //   const cycleDataToSave = { csu11, csu12 };

// //   const cycleDataJson = JSON.stringify(cycleDataToSave, null, 2);
...
// //         testerVoltage: cycle[`cell_${idx}`]?.individual?.receivedVoltage ?? null,
// //       }));
...
//         // Extract the cell number from the cellKey (e.g., "cell_0" -> 0, "cell_12" -> 12)
...
//   return (
//     <div className="w-80 mx-2 p-2 bg-white shadow-lg rounded-xl space-y-4 border border-cyan-100">
...
//         </span>
//         {/* Map to individual tester voltage */}
...
const SerialTerminal: React.FC<SerialTerminalProps> = ({ 
  ...
  const {
    ...
    setCellData: setContextCellData,
    criticalState,
    csu1Statuses,
    csu2Statuses,
    daisyStatuses,
  ...
  useEffect(() => {
    cellDataRef.current = cellData;
    setContextCellData(cellData); // Sync local cellData with context
  }, [cellData, setContextCellData]);
  ...
  useEffect(() => {
    ...
    const initialCellData = Array.from({ length: 24 }, (_, i) => ({
      ...
      temperature: null,
      setVoltage: null,
      expectedVoltage: null, // Initialize expectedVoltage
      setTemperature: null,
      balancing: false,
      openWire: false,
      delay: null,
    ...
    setCellData(initialCellData);
    setContextCellData(initialCellData); // Initialize context cellData
    console.log("SerialTerminal: Initialized cellData:", initialCellData);
  }, [setContextCellData]);
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setCellData(newCellData);
      setContextCellData(newCellData); // Sync with context
    ...
    try {
      ...
      for (let cycle = 1; cycle <= cycleCount; cycle++) {
        ...
        for (let i = 0; i < hexLines.length; i++) {
          ...
          // Update cellData for set_voltage command
          if (command === "set_voltage") {
              ...
              setCellData((prev) => {
                ...
                if (cellNo >= 0 && cellNo <= 23) {
                  newData[cellNo] = {
                    ...
                    voltage: null, // Clear actual voltage until we receive it
                    csu11Voltage: null,
                    csu12Voltage: null,
                    dcCsuVoltage: null,
                  };
                }
                // setContextCellData(newData); // Sync with context
                return newData;
              });
            }
          }
              ...
              (cell) =>
                (cell.voltage !== null ||
                  ...
                  >
                    ...
                    {(cell.voltage !== null || cell.setVoltage !== null || cell.expectedVoltage !== null) && (
                      <div className="ml-4">
                        <span className="font-medium">Individual:</span>
                        {cell.voltage !== null && (
                          <span className="text-green-600">
                        ...
                        {cell.setVoltage !== null && (
                          <span className="text-blue-600">
                            {" "}
                            Set: {cell.setVoltage} V
                          </span>
                        )}
                        {cell.expectedVoltage !== null && (
                          <span className="text-blue-600">
                            {" "}
                            Expected: {cell.expectedVoltage} V
                          </span>
                        )}
                      </div>
                    )}
