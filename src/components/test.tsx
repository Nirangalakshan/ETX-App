...
const SerialTerminal: React.FC<SerialTerminalProps> = ({
  ...
  const updateCellStatesFile = async (
    ...
    try {
      ...
      if (actualVoltage === null || isNaN(actualVoltage)) {
        console.warn(
          `Skipping file update: Invalid actualVoltage for cell ${cellNo}, type ${type}`
        );
        return {
          success: false,
          error: `Invalid actualVoltage: ${actualVoltage}`,
        };
      }
      ...
      console.log(
        `Appending to cell_states_${sanitizedTestName}.json:`,
        cycleData
      );

      const result = await window.fileAPI.updateCellStatesFile(
        cycleData,
        `cell_states_${sanitizedTestName}.json`
      );

      if (result.success) {
        console.log(
          `Successfully appended cycle ${validCycleNo}, type ${type}, cell ${cellNo} to cell_states_${sanitizedTestName}.json`
        );
        setReceived((prev) => ({
          ...prev,
          Individual: [
    ...
    try {
      ...
      const sanitizedTestName =
        testName
          ...
          .toLowerCase() ||
        `test_${new Date().toISOString().replace(/[:.]/g, "-")}`;
      const result = await window.fileAPI.resetCellStates(
        `cell_states_${sanitizedTestName}.json`
      );
      if (result.success) {
        console.log(`Successfully reset cell_states_${sanitizedTestName}.json`);
  ...
  useEffect(() => {
    ...
    const handleCellStateUpdate = async (
      ...
      console.log(
        `handleCellStateUpdate called: cycleNo=${cycleNo}, type=${type}, cellNo=${cellNo}, setVoltage=${setVoltage}, testerVoltage=${testerVoltage}, actualVoltage=${actualVoltage}`
      );
      // For csu11 type and cellNo 0–11, use setVoltage from cellNo + 12
      const mappedCellNo =
        type === "csu11" && cellNo >= 0 && cellNo <= 11
          ? cellNo + 12 <= 23
            ? cellNo + 12
            : 23
          : cellNo;
      const mappedCell = cellDataRef.current[mappedCellNo];
      const setVoltageForCsu11 =
        type === "csu11" && cellNo >= 0 && cellNo <= 11
          ? mappedCell?.setVoltage ?? setVoltage ?? 0
          : setVoltage;
      const result = await updateCellStatesFile(
        cycleNo,
        type,
        cellNo,
        setVoltageForCsu11,
      ...
      if (!isNaN(cellNo) && cellNo >= 0 && cellNo <= 23) {
          ...
          const newCellData = prev.map((cell) =>
            cell.id === cellNo
              ? {
                  ...
                  expectedVoltage:
                    command === "set_voltage" && parsedValue
                      ? EXPECTED_SENT_VOLTAGES[parseInt(parsedValue)] ||
                        cell.expectedVoltage
                      : cell.expectedVoltage,
                  temperature:
                    command === "get_temp" && parsedValue
        ...
        if (
          ...
          if (!isNaN(actualVoltage)) {
            ...
            const individualVoltage = cell.voltage; // Use individual actual voltage
            console.log(
              `Triggering cell state update for ${type}, cell ${cellNo}, voltage ${actualVoltage}, testerVoltage ${individualVoltage}`
            );
            handleCellStateUpdate(
              currentCycle || 1,
              type,
            ...
            console.warn(
              `Invalid actualVoltage for ${type}, cell ${cellNo}: ${parsedValue}`
            );
          }
        }
      }
      ...
      for (const [index, entry] of json.entries()) {
        ...
        if (commandCode === "03") {
          if (command === "set_voltage") {
            ...
            if (cellNo !== undefined && cellNo >= 0 && cellNo <= 23) {
              newCellData[cellNo].setVoltage = value;
              newCellData[cellNo].expectedVoltage =
                EXPECTED_SENT_VOLTAGES[value] || null;
              newCellData[cellNo].voltage = null;
            }
          } else if (command === "set_temp") {
            const value = parseFloat(entry.temperature);
        ...
        for (let i = 0; i < hexLines.length; i++) {
          ...
          if (command === "set_voltage") {
            ...
            const expectedVoltage =
              EXPECTED_SENT_VOLTAGES[voltageHexValue] || null;
          ...
          if (
            ...
            const mappedCell = cellDataRef.current[mappedCellNo];
            const setVoltageForCsu11 =
              mappedCell?.setVoltage ?? cell?.setVoltage ?? 0;
            await updateCellStatesFile(
              cycle,
              "csu11",
              cellNo,
    ...
    cycleData.forEach((cycle, cycleIndex) => {
      const csu11 = Object.entries(cycle)
        ...
        .map(([cellKey, cellValue], idx) => ({
          ...
          csu11Voltage: cellValue?.csu11?.receivedVoltage ?? null,
          testerVoltage1:
            cycle[`cell_${idx + 12}`]?.individual?.receivedVoltage ?? null,
        }));
      ...
      const csu12 = Object.entries(cycle)
        ...
        .map(([cellKey, cellValue], idx) => ({
          ...
          csu12Voltage: cellValue?.csu12?.receivedVoltage ?? null,
          testerVoltage2:
            cycle[`cell_${idx}`]?.individual?.receivedVoltage ?? null,
        }));
      ...
      const dcCsu = Object.entries(cycle)
        ...
        .map(([cellKey, cellValue]) => {
          ...
          return {
            ...
            dcCsuVoltage: cellValue?.dcCsu?.receivedVoltage ?? null,
            testerVoltage:
              cycle[`cell_${cellNumber}`]?.individual?.receivedVoltage ?? null,
          };
        });
          ...
          <div className="flex items-end gap-2">
            <div className="flex flex-col tablet:flex-1">
              ...
              <select
                ...
                onChange={(e) => setSelectedPort(e.target.value)}
                disabled={connectedPorts.includes(selectedPort) || isLoading}
                className="w-full border border-gray-300 rounded-lg px-2 py-1.5 text-sm focus:ring-2 focus:ring-blue-500 disabled:bg-gray-100 transition-colors"
              >
                {availablePorts.length === 0 && connectedPorts.length === 0 ? (
                  <option value="">No ports available</option>
                ) : (
                  [...availablePorts, ...connectedPorts].map((port) => (
                    <option key={port} value={port}>
                      {port}{" "}
                      {connectedPorts.includes(port) ? "(Connected)" : ""}
                    </option>
                  ))
                )}
              </select>
        ...
        {hexLines.length > 0 && (
          <div className="flex flex-wrap gap-2 h-auto">
            <button
              ...
              disabled={
                !connectedPorts.includes(selectedPort) || isLoading || isRunning
              }
              className="flex-1 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg shadow-sm disabled:opacity-50 flex items-center justify-center text-sm transition-colors font-inter"
            >
              {isLoading ? (
                            ...
                            <div key={cellKey} className="ml-4">
                              ...
                              {cellValue.individual.receivedVoltage !==
                                ...
                                <span className="text-green-600">
                                  {" "}
                                  Received:{" "}
                                  {cellValue.individual.receivedVoltage} V
                                </span>
                              )}
                              {cellValue.individual.expectedVoltage !==
                                ...
                                <span className="text-blue-600">
                                  {" "}
                                  Expected:{" "}
                                  {cellValue.individual.expectedVoltage} V
                                </span>
                              )}
                            </div>
                          ) : null
                            ...
                            cellValue?.csu11 ? (
                              <div key={cellKey} className="ml-4">
                                ...
                                <span className="text-purple-600">
                                  {" "}
                                  CSU11 Voltage:{" "}
                                  {cellValue.csu11.receivedVoltage} V
                                </span>
                                {cycle[`cell_${idx + 12}`]?.individual
                                  ?.receivedVoltage !== undefined && (
                                  <span className="text-green-600 ml-2">
                                    Tester:{" "}
                                    {
                                      cycle[`cell_${idx + 12}`].individual
                            ...
                            cellValue?.csu12 ? (
                              <div key={cellKey} className="ml-4">
                                ...
                                <span className="text-orange-600">
                                  {" "}
                                  CSU12 Voltage:{" "}
                                  {cellValue.csu12.receivedVoltage} V
                                </span>
                                {cycle[`cell_${idx}`]?.individual
                                  ?.receivedVoltage !== undefined && (
                                  <span className="text-green-600 ml-2">
                                    Tester:{" "}
                                    {
                        ...
                        {Object.entries(cycle).map(([cellKey, cellValue]) =>
                          cellValue?.dcCsu ? (
                            <div key={cellKey} className="ml-4">
                              ...
                              {cycle[cellKey]?.individual?.receivedVoltage !==
                                undefined && (
                                <span className="text-green-600 ml-2">
                                  Tester:{" "}
                                  {cycle[cellKey].individual.receivedVoltage} V
                                </span>
                              )}
                            </div>
                          ) : null
        ...
        {showCriticalErrorPopup && (
          ...
          >
            ...
            <p className="text-base">
              Test stopped due to critical cell status!
            </p>
            <div className="mt-3">
              <strong>Critical Cells:</strong>
              <ul className="list-disc pl-5 mt-1 text-sm">
