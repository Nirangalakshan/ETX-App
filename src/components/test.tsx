...
//   const id = hexArray[1];
//   const dcIc = hexArray[2]; // DC IC number (00 or 01)
//   const cellNo = hexArray[3];
//   const valueBytes = hexArray.slice(4, 6);
//   const valueInt = (valueBytes[0] << 8) | valueBytes[1];
//   const floatValue = valueInt / 10000;
...
//   const [csu1ResponseData, setCsu1ResponseData] = useState<Record<number, ResponseData[]>>({});
//   const [csu2ResponseData, setCsu2ResponseData] = useState<Record<number, ResponseData[]>>({});
//   const [daisyChainResponseData, setDaisyChainResponseData] = useState<Record<number, ResponseData[]>>({});
//   const [ports, setPorts] = useState<string[]>([]);
//   const [selectedPort, setSelectedPort] = useState<string>("");
//   const [baudRate, setBaudRate] = useState<number>(() => {
//     const saved = localStorage.getItem("serialBaudRate");
...
//           const { cellNo: extractedCellNo, value } = extractDaisyChainData(hexArray);
//           cellNo = extractedCellNo;
//           parsedValue = value;
//           console.log(
...
//         });
//       } else if (command === "daisy_chain") {
//         setDaisyChainResponseData((prevData) => {
//           const existingDataForCell = prevData[cellNo] || [];
//           const commandExists = existingDataForCell.some(
//             (item) => item.command === command
//           );
...
//             [cellNo]: [...existingDataForCell, newResponseEntry],
//           };
//           window.dispatchEvent(new CustomEvent('daisyChainCellsUpdate', { detail: newData }));
//           return newData;
//         });
//       } else if (command === "get_dc_csu_volt" || command === "get_dc_csu_temp" || command === "get_dc_csu_balance" || command === "get_dc_csu_ow") {
//         const dcIc = hexArray[2]; // Use DC IC number (00 or 01) as a key
...
//             },
//           };
//           window.dispatchEvent(new CustomEvent('dcCsuCellsUpdate', { detail: newData }));
//           return newData;
//         });
//       } else {
//         setResponseData((prevData) => {
...
//           if (
//             isNaN(cellNo) ||
//             cellNo < cellNoRange[0] || cellNo > cellNoRange[1]
//           ) {
//             setError(
//               `Invalid cellNo for ${command}: ${entry.cellNo || "undefined"}`
//             );
...
//             if (isNaN(value) || value < -20 || value > 100) {
//               setError(
//                 `Invalid temperature for ${command}: ${entry.temperature ?? "undefined"}`
//               );
//               continue;
//             }
//             frame[4] = value & 0xff;
...
//     CSU11: csu1ResponseData,
//     CSU12: csu2ResponseData,
//     DaisyChain: daisyChainResponseData,
//   };
...
//         Object.keys(csu1ResponseData || {}).length > 0 ||
//         Object.keys(csu2ResponseData || {}).length > 0 ||
//         Object.keys(daisyChainResponseData || {}).length > 0) && (
//         <div className="space-y-2">
//           <label className="text-xs font-medium text-gray-700">Cell Data</label>
//           <div className="h-40 overflow-y-auto border border-gray-300 rounded-lg p-3 bg-gray-50 font-mono text-sm shadow-inner">
//             {Object.entries(allCellData).map(([category, data]) => {
...
//                       console.log(`SerialTerminal: Rendering ${category} Cell ID: ${cellIdOrDcIc}`);
//                       return (
//                         <div key={cellIdOrDcIc} className="text-gray-800 break-all mb-2">
//                           <div className="font-semibold text-blue-600">
//                             {category === "DC CSU" ? `DC IC ${cellIdOrDcIc}` : `Cell ${cellIdOrDcIc}`}
//                           </div>
...
//                                 </div>
//                                 {items.map((item, idx) => (
//                                   <div key={idx} className="ml-8 flex justify-between">
//                                     <span className="text-xs">
//                                       {item.command
//                                         .replace("set_", "")
//                                         .replace("get_", "")
