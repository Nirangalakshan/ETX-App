...
const CSU2: React.FC = () => {
    ...
    () => (cellId: number) => {
      ...
      const voltage =
        ...
        !isNaN(parseFloat(voltageData.value))
          ? parseFloat(voltageData.value)
          : null;
      console.log(
        `[${new Date().toISOString()}] Cell ${cellId} Expected Voltage: ${voltage}`
      );
      return voltage;
    },
    [responseData]
    ...
    () => (cellId: number, dataItems: ResponseData[]) => {
      ...
      const voltage =
        voltageItems.length > 0
          ...
          : null;
      // console.log(
      //   `[${new Date().toISOString()}] Cell ${cellId} Actual Voltage: ${voltage}`
      // );
      return voltage;
    },
    []
  ...
  const getDisplayVoltage = useMemo(
    () => (cellId: number, dataItems: ResponseData[]) => {
      ...
      // console.log(
      //   `[${new Date().toISOString()}] Cell ${cellId} Display Voltage: ${voltage}`
      // );
      return voltage;
    },
    [cellStates]
  ...
  useEffect(() => {
    const debounce = setTimeout(() => {
      if (resetStatus && Object.keys(csu2ResponseData).length === 0) {
        ...
        console.log(
          `[${new Date().toISOString()}] CSU2 reset (actual voltages)`
        );
        return;
      }

      if (
        JSON.stringify(csu2ResponseData) !==
        JSON.stringify(prevCsu2ResponseData.current)
      ) {
        console.log(
          `[${new Date().toISOString()}] CSU2 Actual Voltage Data:`,
          csu2ResponseData
        );
        prevCsu2ResponseData.current = csu2ResponseData;
        ...
        setCellStates((prev) => {
          const updatedCellStates = prev.map((state, cellId) => {
            ...
            if (latestItem && latestItem.command === "get_dc_csu_volt") {
              ...
              console.log(
                `[${new Date().toISOString()}] Cell ${cellId} Disconnect Detected - Resetting state`
              );
              return newState;
            } else {
              ...
              if (
                actualVoltageObj &&
                (prevActual.value !== actualVoltageObj.value ||
                  prevActual.timestamp !== actualVoltageObj.timestamp)
              ) {
                const diff =
                  expectedVoltage !== null
                    ? Math.abs(
                        parseFloat(actualVoltageObj.value) - expectedVoltage
                      )
                    : 0;
                const newState = {
                  ...state,
                  actual: actualVoltageObj,
                  expected:
                    diff > DIFF_RESET_THRESHOLD ? null : expectedVoltage,
                  status: diff > DIFF_RESET_THRESHOLD ? null : state.status,
                  hideTester: diff > DIFF_RESET_THRESHOLD,
                };
                console.log(
                  `[${new Date().toISOString()}] Cell ${cellId} Actual Voltage Updated - Actual: ${
                    actualVoltageObj.value
                  }, Expected: ${newState.expected}, Status: ${
                    newState.status
                  }, HideTester: ${newState.hideTester}`
                );
                return newState;
              }
          ...
          const statuses = updatedCellStates.map((cellState, cellId) => {
            const dataItems = csu2ResponseData[cellId] || [];
            const status =
              cellState.hideTester || cellState.expected === null
                ? "N/A"
                : getCellStatus(dataItems, cellState.expected);
            const actualVoltage = getActualVoltage(cellId, dataItems);
            const expectedVoltage = cellState.expected;
          ...
          setCsu2Statuses(statuses);
          console.log(
            `[${new Date().toISOString()}] Set csu2Statuses:`,
            statuses
          );
          const hasCritical = statuses.some((s) => s.status === "critical");
          setCriticalState(hasCritical);
          console.log(
            `[${new Date().toISOString()}] Updated Cell States (actual):`,
            updatedCellStates
          );
  ...
  useEffect(() => {
    const debounce = setTimeout(() => {
      if (resetStatus && Object.keys(csu2ResponseData).length === 0) {
        ...
        // console.log(
        //   `[${new Date().toISOString()}] CSU2 reset (tester voltages)`
        // );
        return;
      }
      ...
      if (
        ...
        JSON.stringify(prevResponseData.current)
      ) {
        console.log(
          `[${new Date().toISOString()}] CSU2 Tester Voltage Data:`,
          responseData
        );
        prevResponseData.current = responseData;
        ...
        setCellStates((prev) => {
          const updatedCellStates = prev.map((state, cellId) => {
            ...
            const dataItems = csu2ResponseData[cellId] || [];
            const status =
              state.hideTester || expectedVoltage === null
                ? "N/A"
                : getCellStatus(dataItems, expectedVoltage);
            ...
            console.log(
              `[${new Date().toISOString()}] Cell ${cellId} Tester Voltage Updated - Expected: ${
                newState.expected
              }, Status: ${newState.status}, HideTester: ${newState.hideTester}`
            );
            return newState;
          });
          ...
          setCriticalState(hasCritical);
          console.log(
            `[${new Date().toISOString()}] Updated Cell States (tester):`,
            updatedCellStates
          );
  ...
  return (
    <div className="p-3 sm:p-4 bg-gray-50 w-full h-full lg:min-w-[17vw] max-w-6xl xl:max-w-7xl mx-auto shadow-md flex justify-center border border-cyan-100 rounded-lg overflow-y-auto max-h-[59vh]">
      <div className="w-full  max-w-6xl relative">
        <h2 className="text-lg font-inter text-gray-800 mb-3 text-center font-semibold py-1 rounded-md shadow-md">
          CSU12
        </h2>
          ...
          {rows.map((row, rowIndex) => (
            ...
            >
              {row.map((cellId) => {
                ...
                const expectedVoltage = state.expected;
                const displayVoltage = getDisplayVoltage(cellId, dataItems);
                const currentActualVoltage = getActualVoltage(
                  cellId,
                  dataItems
                );
                const status =
                      ...
                      <div className="flex justify-between">
                        ...
                        <span
                          className={
                            ...
                            displayVoltage !== "-" &&
                            (parseFloat(displayVoltage) > 4.5 ||
                              (parseFloat(displayVoltage) < 2.0 &&
                                displayVoltage !== "1"))
                              ? "text-red-600"
                              : currentActualVoltage === null &&
                                displayVoltage !== "-"
                              ? "text-gray-500 italic"
                              : ""
                          }
                        ...
                        >
                          ...
                          <div className="space-y-1.5">
                            {[
                              ...
                              .map((item, idx) => {
                                const currentActual =
                                  cellStates[selectedCell]?.actual;
                                const isLatest =
                                  currentActual &&
                                  currentActual.timestamp === item.timestamp;
                                return (
                                  <div
                                    key={idx}
