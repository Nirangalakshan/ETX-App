...
const ErrorWarningPanel: React.FC<ErrorWarningPanelProps> = ({ csu1Cells, csu2Cells, daisyChainData }) => {
  ...
  useEffect(() => {
    console.log('ErrorWarningPanel: useEffect triggered', { allCells, daisyChainData });
  }, [allCells, daisyChainData]);
  ...
  return (
    <div className="w-full bg-white border border-gray-200 rounded-xl p-5 shadow-md mt-4 space-y-4 overflow-y-auto" style={{ height: 'calc(22vh - 20px)' }}>
      ...
      {allCells.filter(cell => cell.status === 'critical').length > 0 && (
        <div>
          <h3 className="text-lg font-semibold text-red-600 mb-2 flex items-center gap-2">
            🔴 Critical Errors <span className="text-xs bg-red-100 text-red-700 px-2 py-0.5 rounded-full">{allCells.filter(cell => cell.status === 'critical').length}</span>
          </h3>
          <ul className="space-y-2">
            {allCells
              ...
              .map((cell) => (
                ...
                >
                  ...
                  Voltage: <span className="font-medium">{cell.voltage !== null ? cell.voltage.toFixed(2) : '-'}</span>,
                  Temp: <span className="font-medium">{cell.temperature !== null ? cell.temperature.toFixed(1) : '-'}</span>
                  {cell.balancing && <><br />Balancing: On</>}
                  {cell.openWire && <><br />Open Wire: Detected</>}
                </li>
              ))}
          </ul>
        </div>
      ...
      {allCells.filter(cell => cell.status === 'warning').length > 0 && (
        <div>
          <h3 className="text-lg font-semibold text-yellow-600 mb-2 flex items-center gap-2">
            🟡 Warnings <span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full">{allCells.filter(cell => cell.status === 'warning').length}</span>
          </h3>
          <ul className="space-y-2">
            {allCells
              .filter(cell => cell.status === 'warning')
              .map((cell) => (
                <li
                  key={`warning-${cell.id}`}
                  className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 text-sm text-yellow-800 shadow-sm"
                >
                  <strong>Cell {cell.id + 1}</strong> (CSU {cell.id < 12 ? 1 : 2})<br />
                  Voltage: <span className="font-medium">{cell.voltage !== null ? cell.voltage.toFixed(2) : '-'}</span>,
                  Temp: <span className="font-medium">{cell.temperature !== null ? cell.temperature.toFixed(1) : '-'}</span>
                  {cell.balancing && <><br />Balancing: On</>}
                  {cell.openWire && <><br />Open Wire: Detected</>}
