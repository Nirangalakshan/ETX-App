...
// // Define threshold values for voltage and temperature gaps
...
// // Define expected sent voltage values for each cell (based on user feedback)
...
//     // Use expected sent voltage if available, otherwise use setVoltage from cell data
//     const expectedVoltage = EXPECTED_SENT_VOLTAGES[cell.id + 1]; // cell.id is 0-based, our mapping is 1-based
//     const sentVoltage = expectedVoltage || cell.setVoltage;
    
//     if (sentVoltage !== null && cell.voltage !== null) {
...
//     // Process CSU1 and CSU2 cells - include original status and gap checking
//     const newErrors = allCells.filter(cell => cell.status === 'critical');
//     const newWarnings = allCells.filter(cell => cell.status === 'warning');

//     // Check for voltage and temperature gaps
//     const gapIssues: { type: string; cellId: number; status: CellStatus; details: string }[] = [];
...
//       ...newErrors,
//       ...gapIssues.filter(issue => issue.status === 'critical'),
//       ...daisyChainIssues.filter(issue => issue.status === 'critical'),
//     ];
//     const combinedWarnings = [
//       ...newWarnings,
//       ...gapIssues.filter(issue => issue.status === 'warning'),
...
const ErrorWarningPanel: React.FC<ErrorWarningPanelProps> = ({ csu1Cells, csu2Cells, daisyChainData }) => {
  ...
  // Helper function to check voltage gap
  const checkVoltageGap = (cell: BatteryCell): { status: CellStatus; details?: string } => {
    const expectedVoltage = EXPECTED_SENT_VOLTAGES[cell.id + 1];
    const sentVoltage = expectedVoltage !== undefined ? expectedVoltage : cell.setVoltage;
